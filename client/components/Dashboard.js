...
export default function Dashboard(props) {
  ...
  useEffect(() => {
    axios.get('http://localhost:3001/account').then((AccOfUserID) =>{
        ...
        <TableBody>
          {AccountsList.map((Account) => (
            <StyledTableRow key={Account.accountNo}>
              <StyledTableCell align="left">{Account.accountNo}</StyledTableCell>
              <StyledTableCell align="left">{Account.balance}</StyledTableCell>
              <StyledTableCell align="left">
                <button type="button" onClick={() => {
                  router.push({
                    pathname: `http://localhost:3001/transactions/[AID]`,
                    query: {AID: Account.accountNo},
                  })
                }}>
                  {Account.accountNo}
                </button>
